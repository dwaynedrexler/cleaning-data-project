...
## Download the data file - if it hasn't already been downloaded
...
for (i in 1:length(colReplace)) {
        newNames <- gsub(names(colReplace)[i], colReplace[i], newNames)
}
names(data)     <- newNames

## -----
## STEP 5. "From the data set in Step 4, creates a second, independent tidy data set with the average of each variable for each activity and each subject."
## A function to summarise each of the columns by Activity - using the mean as the summarisation function
get_averages <- function(d) { d %>% group_by(Activity) %>% summarise_each(funs(mean)) }
## Split the data by Subject
split_data <- split(data, data$Subject)
## Run each Subject's data through the 'get_averages' function
final <- lapply(split_data, get_averages)
## Re-order the first two columns - as Subject becomes the 2nd column after running through the get_averages function
...
result <- data.frame(matrix(ncol = 81, nrow = 0))
